...
export default class SwimlaneLayoutGraph extends React.PureComponent<IProps, any> {
    ...
    initGraph = () => {
        ...
        editor.setGraphContainer(this.graphContainer);
        ...
        if (graph.isEnabled()) {
            ...
            graph.isValidSource = function (cell) {
                if (mxGraph.prototype.isValidSource.apply(graph, [cell])) {
            ...
            graph.isValidTarget = function (cell) {
                ...
                return !graph.getModel().isEdge(cell) && !graph.isSwimlane(cell) &&
            ...
            graph.isValidDropTarget = function (target, cells, evt) {
                ...
                for (var i = 0; i < cells.length; i++) {
                    ...
                    lane = lane || graph.isPool(tmp);
                    pool = pool || graph.isPool(cells[i]);

                    cell = cell || !(lane || pool);
                }

                return !pool && cell != lane && ((lane && graph.isPool(target)) ||
                    (cell && graph.isPool(model.getParent(target))));
            };

            // Adds new method for identifying a pool
            graph.isPool = function (cell: mxCell) {
                var model = graph.getModel();
            ...
            graph.model.getStyle = function (cell) {
                var style = mxGraphModel.prototype.getStyle.apply(graph.model, [cell]);
            ...
            var foldingHandler = function (sender: any, evt: any) {
